...
START=$(date +%s.%2N)
#Check if there is a compiled exe. If not, it is an interpreted language, we just need to interpret. 
if [ "$runTarget" = "" ]; then
    $compiler /usercode/$compileTarget -< $"/usercode/inputFile" #| tee /usercode/output.txt
#Compiled language.
else
	#In case of compile errors, redirect them to a file
    $compiler /usercode/$compileTarget $runtimeArgs #&> /usercode/errors.txt
	#The compiler returned 0. Success. 
	if [ $? -eq 0 ];	then
		$runTarget -< $"/usercode/inputFile" #| tee /usercode/output.txt    
	#The compiler didn't return 0. Compilation failed.
	else
